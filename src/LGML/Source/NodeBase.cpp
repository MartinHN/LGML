...
//    suspendProcessing(false);
  ...
  if (!isSuspended())
  {
    ...
    // crossfade if we have a dry mix i.e at least one input channel 
      if(crossfadeValue!=1 && crossFadeBuffer.getNumChannels()>0){
        for(int i = 0 ; i < getTotalNumOutputChannels() ; i++){
          ...
          int maxCommonChannels = jmin(crossFadeBuffer.getNumChannels()-1,i);
          buffer.addFromWithRamp(i, 0, crossFadeBuffer.getReadPointer(maxCommonChannels), (float)numSample, (float)lastDryVolume,(float)curDryVolume);
    ...
    lastDryVolume = (float)curDryVolume;
